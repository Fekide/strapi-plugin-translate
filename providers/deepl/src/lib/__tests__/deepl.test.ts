...
describe('deepl provider', () => {
  ...
  const isAuthenticated = (req: StrictRequest<DefaultBodyType>) => {
  ...
  const usageHandler: HttpResponseResolver<
    PathParams,
    DefaultBodyType,
    undefined
  > = async ({ request }) => {
  ...
  const translateHandler: HttpResponseResolver<
    PathParams,
    DefaultBodyType,
    undefined
  > = async ({ request }) => {
  ...
  describe('usage', () => {
    describe.each([
      ...
      'for free api %p, with key valid %p',
      (freeApi: boolean, validKey: boolean) => {
        let deeplProvider: InitializedProvider
          ...
          describe('succeeds', () => {
            ...
            it('with a field larger than request size limit', async () => {
              ...
              const params = {
                ...
                targetLocale: 'de',
                text: [
                  faker.lorem.paragraphs(DEEPL_API_MAX_REQUEST_SIZE / 200),
                ],
              }
              // when
              const result = await deeplProvider.translate(params)
    ...
    describe('provider options', () => {
      ...
      describe('api options used', () => {
        const registerHandlerEnforceParams = (
          requiredParams: TranslateTextOptions,
          forbiddenParams: Array<keyof TranslateTextOptions> = []
        ) => {
          const handler: HttpResponseResolver<
            PathParams,
            DefaultBodyType,
            undefined
          > = async ({ request }) => {
